...
Visualize cloud infrastructure. Use pre-built dashboards — for AWS, Azure, GCP, Kubernetes, and more — to visualize your cloud resources and answer common questions about resource quantity, cost, usage, and relationships.

Run security and compliance benchmarks. Use pre-built benchmarks to assess how well your clouds comply with the standard frameworks including CIS, GDPR, NIST, PCI, SOC 2, and more.

Create your own dashboards and benchmarks. Build from scratch, using HCL in a live coding environment, or compose with the prebuilt dashboards and benchmarks.

Pre-requisites
To get started, you will need to install Powerpipe, Steampipe, and the AWS plugin for Steampipe.

Powerpipe does not require Steampipe; it can work with any PostgreSQL, MySQL, SQLite, or DuckDB database. Mods are written for a specific SQL syntax and database schema, however, and the examples in this article require a Steampipe database with the AWS plugin.
...
Next, install Steampipe.


Linux
sudo /bin/sh -c "$(curl -fsSL https://steampipe.io/install/steampipe.sh)"
Now that Steampipe is installed, install & configure the AWS plugin for Steampipe

steampipe plugin install aws
Out of the box, Steampipe will use the default AWS credentials from your credential file and/or environment variables; if you can run aws ec2 describe-vpcs, for example, then you should be able to run the examples. The AWS plugin documentation provides additional examples to configure your credentials, and you can even configure Steampipe to query multiple accounts and multiple regions.

Start the Steampipe service. The Steampipe database needs to be running for Powerpipe to connect to it:
...
Powerpipe always runs in the context of a mod. A Powerpipe mod is a portable, versioned collection of related Powerpipe resources such as dashboards, benchmarks, queries, and controls defined in HCL, and distributed as simple text files. Powerpipe loads the mod from the mod location which defaults to the current directory.
...
Dashboards provide a wealth of information about your resources using charts, graphs, and tables. You can change the search path, take a snapshot once the benchmark is complete, see details in the panel view, and download the results in a CSV file.
...
The Powerpipe Hub contains hundreds of ready-made dashboards and benchmarks that you can simply install and run. But Powerpipe also makes it simple to write your controls and benchmarks, and build dashboards to analyze your data and share with others!

We've already created our own mod and installed 2 dependency mods (AWS Insights and AWS Compliance). Now let's add our own dashboard and benchmark. Create a file named learn.pp in your mod's directory, and paste in the following HCL code:
